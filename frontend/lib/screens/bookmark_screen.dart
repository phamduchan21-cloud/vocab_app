...
class _BookmarkScreenState extends State<BookmarkScreen> {
  ...
  /// Convert all loaded Vocabulary items into _BookmarkWord and filter by tab & search.
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            // ─── Topbar ──────────────────────────────────────────────
            Padding(
              padding: EdgeInsets.fromLTRB(MediaQuery.of(context).size.width < 400 ? 20 : 44, 36, MediaQuery.of(context).size.width < 400 ? 20 : 44, 0),
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${provider.items.length} từ bạn đã đánh dấu để ôn lại sau',
                          style: GoogleFonts.workSans(
                            fontSize: 15,
            ...
            // ─── Body ────────────────────────────────────────────────
            Expanded(
              child: _buildBody(isLoading, hasError, isEmptyState, provider),
            ),
          ],
        ),
      ),
  ...
  Widget _buildBody(
    ...
    return SingleChildScrollView(
      padding: EdgeInsets.fromLTRB(MediaQuery.of(context).size.width < 400 ? 20 : 44, 0, MediaQuery.of(context).size.width < 400 ? 20 : 44, 60),
      child: LayoutBuilder(
      ...
      for (var j = i; j < rowEnd; j++) {
        rowItems.add(
          SizedBox(
            ...
            child: _WordStamp(
              word: words[j],
              isStarred: _bookmarkedIds.contains(words[j].id),
              onStarToggle: () => _toggleBookmark(words[j].id),
            ),
          ),
        );
      }
...
// ─── Skeleton Loading for bookmark grid ──────────────────────────────
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          const spacing = 14.0;
          final crossAxisCount = constraints.maxWidth < 500 ? 3 : 4;
          final itemWidth =
              (constraints.maxWidth - spacing * (crossAxisCount - 1)) /
                  crossAxisCount;
          final rowsCount = (count + crossAxisCount - 1) ~/ crossAxisCount;
              ...
              return Padding(
                ...
                child: Row(
                  children: List.generate(crossAxisCount, (colIndex) {
                    final idx = rowIndex * crossAxisCount + colIndex;
                    if (idx >= count) return const Spacer();
                    return Padding(
                      padding: EdgeInsets.only(
                        left: colIndex == 0 ? 0 : spacing / 2,
                        right: colIndex == crossAxisCount - 1 ? 0 : spacing / 2,
                      ),
                      child: Container(
                        width: itemWidth,
                        height: 148,
...
class _WordStamp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    switch (word.tag) {
      case _WordTag.new_:
        tagBg = AppColors.warning.withValues(alpha: 0.15);
        tagFg = AppColors.warning;
      case _WordTag.learning:
        tagBg = AppColors.blue.withValues(alpha: 0.12);
        tagFg = AppColors.blue;
      case _WordTag.mastered:
        tagBg = AppColors.success.withValues(alpha: 0.12);
        tagFg = AppColors.success;
    }
