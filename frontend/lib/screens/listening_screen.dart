...
class _ListeningScreenState extends State<ListeningScreen> {
  ...
  static const _typeColors = {
    'correct_answer': Color(0xFF8B5CF6),
    'next_sentence': Color(0xFF34D399),
    'find_place': Color(0xFFF472B6),
    'topic': Color(0xFFFB923C),
    'choose_image': Color(0xFF3B82F6),
    'content': Color(0xFFEF4444),
    'thought': Color(0xFF14B8A6),
    'short_dialogue': Color(0xFFA855F7),
  };
  ...
  Future<void> _loadData() async {
    ...
    try {
      final typesRes = await http.get(
        Uri.parse('${ApiConfig.baseUrl}/api/listening/types'),
      );
      final progressRes = await http.get(
        Uri.parse('${ApiConfig.baseUrl}/api/listening/progress'),
      );
      if (typesRes.statusCode == 200 && progressRes.statusCode == 200) {
        final typesData = json.decode(typesRes.body);
        final progressData = json.decode(progressRes.body);
        setState(() {
          _types = List<Map<String, dynamic>>.from(typesData['types']);
  ...
  void _mockData() {
    setState(() {
      _isLoading = false;
      _types = [
        {'id': 'correct_answer', 'title': 'Chọn đáp án đúng', 'icon': '🅰️', 'desc': 'Nghe 1 câu → chọn đáp án'},
        {'id': 'next_sentence', 'title': 'Tìm câu nối tiếp', 'icon': '🔗', 'desc': 'Chọn câu trả lời phù hợp'},
        {'id': 'find_place', 'title': 'Tìm địa điểm', 'icon': '📍', 'desc': 'Chọn địa điểm phù hợp'},
        {'id': 'topic', 'title': 'Chủ đề hội thoại', 'icon': '💬', 'desc': 'Chọn chủ đề đúng'},
        {'id': 'choose_image', 'title': 'Chọn tranh đúng', 'icon': '🖼️', 'desc': 'Chọn bức tranh phù hợp'},
        {'id': 'content', 'title': 'Nội dung hội thoại', 'icon': '📖', 'desc': 'Chọn nội dung đúng'},
        {'id': 'thought', 'title': 'Chọn suy nghĩ', 'icon': '🤔', 'desc': 'Chọn suy nghĩ người nói'},
        {'id': 'short_dialogue', 'title': 'Hội thoại ngắn', 'icon': '💭', 'desc': 'Hội thoại 2-3 câu'},
      ];
      _progress = {'completed': 3, 'correct_answers': 45, 'total_answers': 60, 'accuracy': 75.0};
    });
  }
  ...
  Future<void> _startQuiz(String type) async {
    ...
    try {
      final res = await http.post(
        Uri.parse('${ApiConfig.baseUrl}/api/listening/generate?subtype=$type&count=5'),
      );
    ...
    setState(() {
      _quiz = {
        'questions': List.generate(5, (i) => {
          ...
          'audio_text': 'Audio for question ${i+1}',
        }),
        'total': 5,
      };
      _currentQuestion = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('🎧 Nghe hiểu', style: GoogleFonts.nunito(fontWeight: FontWeight.bold, fontSize: 20)),
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('$_correctCount/${_currentQuestion + (_selectedAnswer != null ? 1 : 0)} đúng',
            ...
            Container(
              ...
              child: Row(children: [
                const Icon(Icons.volume_up_rounded, color: Color(0xFF8B5CF6)),
                const SizedBox(width: 8),
                Text('🎧 Nghe câu hỏi', style: GoogleFonts.nunito(fontSize: 14, color: AppColors.textPrimary)),
              ]),
            ),
            ...
            ...List.generate((q['options'] as List).length, (i) {
              ...
              if (_selectedAnswer != null) {
                optColor = isCorrect ? AppColors.accent3 : (isWrong ? AppColors.accent2 : null);
              }
              return GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: optColor?.withValues(alpha: 0.1) ?? (isSelected ? const Color(0xFF8B5CF6).withValues(alpha: 0.1) : Colors.white),
                    borderRadius: BorderRadius.circular(14),
                    border: Border.all(color: optColor ?? (isSelected ? const Color(0xFF8B5CF6) : const Color(0xFFE5E7EB)), width: isSelected ? 2 : 1),
                  ),
                  child: Row(children: [
                    Container(
                      width: 28, height: 28,
                      decoration: BoxDecoration(shape: BoxShape.circle, color: isSelected ? const Color(0xFF8B5CF6) : Colors.transparent, border: Border.all(color: isSelected ? const Color(0xFF8B5CF6) : AppColors.textHint)),
